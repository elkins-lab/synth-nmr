...
def calculate_rdcs(structure: struc.AtomArray, Da: float, R: float) -> Dict[int, float]:
    ...
    logger.info(f"Calculating RDCs with Da={Da:.2f} Hz, R={R:.3f}")
    ...
    for n_atom in n_atoms:
        ...
        if res_id not in h_coord_map:
            logger.debug(f"No backbone H found for residue {res_id}, skipping RDC calculation.")
            continue
    ...
    if not rdcs and len(n_atoms) > 0:
        logger.warning("No RDCs were calculated. Ensure the structure contains backbone N and corresponding H atoms.")
