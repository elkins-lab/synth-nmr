...
def load_matched_dataset(data_dir: str = "data") -> List[Tuple[struc.AtomArray, Dict[int, Dict[str, float]]]]:
    """
    ...
    for pdb_id, bmrb_id in TRAINING_PAIRS:
        ...
        try:
            ...
            if len(struct) > 0 and len(shifts) > 0:
                ...
                logger.info(f"Loaded {pdb_id}/{bmrb_id}: {struc.get_residue_count(struct)} residues, {len(shifts)} shift assignments")
        except Exception as e:
