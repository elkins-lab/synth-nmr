...
def test_train_gnn_script_execution(mocker, tmp_path):
    ...
    mocker.patch("synth_nmr.scripts.train_gnn.load_matched_dataset", return_value=_make_dummy_dataset())
    
    test_args = ["train_gnn.py", "--epochs", "1", "--batch-size", "1", "--save-path", str(tmp_path / "model.pt")]
