...
from synth_nmr.synth_nmr_cli import process_commands, handle_interactive_command
...
class TestCLIExtended(unittest.TestCase):
    ...
    def test_cli_export_commands(self):
        ...
        args = [
            "read", "pdb", self.pdb_path,
            "export", "nef", "test_export.nef",
            "export", "shifts", "test_export.csv"
        ]
        with patch("sys.stdout", new=StringIO()) as fake_out, \
             patch("synth_nmr.synth_nmr_cli.calculate_synthetic_noes", return_value=[]):
    ...
    def test_cli_validate_noes_mocked(self):
        ...
        with patch("synth_nmr.synth_nmr_cli.download_bmrb_file", return_value="mock.str"), \
             patch("synth_nmr.synth_nmr_cli.parse_bmrb_restraints", return_value={}), \
             patch("synth_nmr.synth_nmr_cli.calculate_synthetic_noes", return_value=[]), \
             patch("synth_nmr.synth_nmr_cli.calculate_rpf_scores", return_value={'recall':0.9, 'precision':0.8, 'f_measure':0.85}), \
             patch("synth_nmr.synth_nmr_cli.calculate_dp_score", return_value=0.7), \
    ...
    def test_cli_main_interactive_trigger(self):
        ...
        with patch("sys.argv", ["synth_nmr_cli"]), \
             patch("synth_nmr.synth_nmr_cli.interactive_mode") as mock_interactive:
            cli.main()
            mock_interactive.assert_called_once()

    def test_cli_main_batch_trigger(self):
        """Test main entry point triggers process_commands when args provided."""
        with patch("sys.argv", ["synth_nmr_cli", "read", "pdb", "file.pdb"]), \
             patch("synth_nmr.synth_nmr_cli.process_commands") as mock_process:
            cli.main()
