...
def test_predict_empirical_shifts_exception_raising(mocker):
    ...
    mocker.patch(
        "biotite.structure.get_residue_starts", side_effect=Exception("Mock Prediction Error")
    )
...
def test_predict_chemical_shifts_shiftx2_empty(mocker):
    ...
    spy_empirical = mocker.patch(
        "synth_nmr.chemical_shifts.predict_empirical_shifts", return_value={"mock": "fallback"}
    )
...
def test_predict_chemical_shifts_shiftx2_exception(mocker):
    ...
    mocker.patch("synth_nmr.chemical_shifts.ShiftX2Predictor.is_available", return_value=True)
    mocker.patch(
        "synth_nmr.chemical_shifts.ShiftX2Predictor.predict", side_effect=Exception("Crash")
    )

    spy_empirical = mocker.patch(
        "synth_nmr.chemical_shifts.predict_empirical_shifts",
        return_value={"mock": "fallback_crash"},
    )
...
def test_predict_chemical_shifts_shiftx2_success(mocker):
    ...
    mocker.patch("synth_nmr.chemical_shifts.ShiftX2Predictor.is_available", return_value=True)
    mocker.patch(
        "synth_nmr.chemical_shifts.ShiftX2Predictor.predict", return_value={"A": {1: {"CA": 50.0}}}
    )
...
def test_shiftx2_predictor_predict_success(mocker):
    ...
    mocker.patch(
        "synth_nmr.chemical_shifts.ShiftX2Predictor._parse_output",
        return_value={"A": {1: {"CA": 50.0}}},
    )
