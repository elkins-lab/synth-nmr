...
def test_exclude_intra_residue(sample_structure):
    ...
    all_restraints = calculate_synthetic_noes(
        sample_structure, cutoff=3.0, exclude_intra_residue=False
    )

    # Then, calculate without intra-residue NOEs
    inter_restraints = calculate_synthetic_noes(
        sample_structure, cutoff=3.0, exclude_intra_residue=True
    )
...
def test_specific_restraint_content(sample_structure):
    ...
    for restraint in restraints:
        ...
        match1 = (
            restraint["atom_name_1"] == "HA1"
            and restraint["res_name_1"] == "GLY"
            and restraint["atom_name_2"] == "H"
            and restraint["res_name_2"] == "ALA"
        )
        match2 = (
            restraint["atom_name_2"] == "HA1"
            and restraint["res_name_2"] == "GLY"
            and restraint["atom_name_1"] == "H"
            and restraint["res_name_1"] == "ALA"
        )
