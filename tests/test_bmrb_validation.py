...
from synth_nmr.structure_utils import get_secondary_structure # Not directly used in this test, but good to keep consistent imports
...
def parse_bmrb_nmr_star_file(filepath):
    ...
    for line in lines:
        ...
        elif len(loop_headers) > 0 and not in_atom_chem_shift_loop and not line.startswith('save_') and not line.startswith('data_'):
            # This is where we transition from reading headers to reading data
            in_atom_chem_shift_loop = True

        if in_atom_chem_shift_loop and not (line.startswith('save_') or line.startswith('loop_') or line.startswith('stop_') or line.startswith('data_')) and len(line) > 0:
            ...
            if len(parts) == len(loop_headers):
                ...
                if atom_element_bmrb == "H" and (atom_id_bmrb == "HA" or atom_id_bmrb.startswith("HA")): # Covers HA, HA2, HA3
                    common_atom_name = "HA"
...
def ubiquitin_structure_1d3z_with_hydrogens() -> struc.AtomArray:
    ...
    for i in range(len(structure)):
        ...
        if atom.atom_name == "N" and atom.res_name != "PRO":
            ...
            ca_indices = np.where((structure.res_id == atom.res_id) & (structure.atom_name == 'CA'))[0]
            
            # Find C of previous residue. Handle N-terminal residue (no previous C)
            c_prev_indices = np.where((structure.res_id == atom.res_id - 1) & (structure.atom_name == 'C'))[0]
...
def test_bmrb_chemical_shift_validation(ubiquitin_structure_1d3z_with_hydrogens: struc.AtomArray, monkeypatch):
    """
    ...
    predicted_shifts = predicted_shifts_all_chains.get('A', {}) # Format: {res_id: {atom_type: shift_value}}

    # Initialize storage for comparison
    experimental_vals_by_atom_type = {
        "HA": [], "CA": [], "N": []
    }
    predicted_vals_by_atom_type = {
        "HA": [], "CA": [], "N": []
    }
    ...
    for i in range(len(ubiquitin_structure_1d3z_with_hydrogens)):
        ...
        if atom_element == "H" and (atom_name == "HA" or atom_name.startswith("HA")): # Covers HA, HA2, HA3 in predicted
        ...
        if mapped_atom_name_for_comparison:
            ...
            if res_id in predicted_shifts and mapped_atom_name_for_comparison in predicted_shifts[res_id]:
                ...
                if res_id in BMRB_EXPERIMENTAL_SHIFTS and mapped_atom_name_for_comparison in BMRB_EXPERIMENTAL_SHIFTS[res_id]:
                    ...
                    if mapped_atom_name_for_comparison == "HA":
                        ha_differences.append((res_id, res_name, exp_val, pred_val, abs(exp_val - pred_val)))
    ...
    for res_id, res_name, exp_val, pred_val, diff in sorted(ha_differences, key=lambda x: x[4], reverse=True)[:5]:
        print(f"Res {res_id} ({res_name}): Abs Diff = {diff:.2f} (Exp: {exp_val:.2f}, Pred: {pred_val:.2f})")
    ...
    for atom_type_key in ["HA", "CA", "N"]:
        ...
        assert rmsd < tolerance_rmsd, f"RMSD for {atom_type_key} shifts is {rmsd:.2f} ppm, which is too high (tolerance: {tolerance_rmsd} ppm)."
        if min_pearson_corr > 0.0: # Only assert correlation for atom types where it's expected
            assert pearson_corr > min_pearson_corr, f"Pearson correlation for {atom_type_key} shifts is {pearson_corr:.2f}, which is too low (minimum: {min_pearson_corr})."
