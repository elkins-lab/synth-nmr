...
def test_interactive_mode_exit(capsys, mocker):
    ...
    assert "Entering interactive mode" not in out # Entering interactive mode is printed by process_commands, not interactive_mode
...
def test_interactive_mode_predict_shifts_valid(capsys, mocker):
    ...
    mocker.patch("synth_nmr.synth_nmr_cli.predict_chemical_shifts", return_value={"A": {1: {"CA": 50.0}}})
