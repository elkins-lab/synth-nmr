...
def align_predictions(synth_preds, shiftx_preds):
    ...
    Returns:
        dict: A mapping of atom types to matched tuples, e.g. 
    ...
    flat_shiftx = {}
    for chain_id, res_dict in shiftx_preds.items():
        for res_id, shifts in res_dict.items():
...
def calculate_metrics(aligned_data):
    ...
    Returns:
        dict: A dictionary of computed metrics for each atom type.
    """
    metrics = {}
        ...
        for method_idx, (method_name, aligned_data) in enumerate(aligned_data_dict.items()):
            ...
            if len(synth_vals) > 1:
                try:
                    ...
                    ax.text(0.05, y_pos, f'{method_name} $r$ = {corr:.2f}', transform=ax.transAxes, 
                            verticalalignment='top', color=c, fontweight='bold')
                except Exception:
                    pass
...
def main():
    """
    Main entry point for the evaluation script. Parses CLI arguments, loads the
    ...
    parser.add_argument("--shiftx2-exe", default="shiftx2.py", 
                        help="Path/command for SHIFTX2 executable")
    parser.add_argument("--plot", default="shift_comparison.png",
    ...
    if not shiftx_predictor.is_available():
        print(f"Error: SHIFTX2 executable '{args.shiftx2_exe}' not found in PATH or not executable.")
        sys.exit(1)
